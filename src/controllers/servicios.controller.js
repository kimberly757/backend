...
exports.create = async (req, res) => {
  try {
    const item = await ServicioModel.create(req.body);
...
exports.update = async (req, res) => {
  try {
    const item = await ServicioModel.update(req.params.id, req.body);
    if (!item) return res.status(404).json({ message: 'No encontrado' });
    res.json(item);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
exports.remove = async (req, res) => {
  try {
    const item = await ServicioModel.remove(req.params.id);
    if (!item) return res.status(404).json({ message: 'No encontrado' });
